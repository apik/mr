...
public:
        ...
        const std::string& legend1, const std::string& legend2, const std::string& legend3, const std::string& legend4, const std::string& legend5) : fname(fname_)
  {
    ...
    plt << "plot '" << fname << ".dat' using 1:2 with lines title label1 lt 1," 
        << "     '" << fname << ".dat' using 1:3 with lines title label2 lt 2,"
        << "     '" << fname << ".dat' using 1:4 with lines title label3 lt 3,"
        << "     '" << fname << ".dat' using 1:5 with lines title label4 lt 4,"
        << "     '" << fname << ".dat' using 1:6 with lines title label5 lt 5 "<< std::endl;
    plt << "" << std::endl;
...
public:
  // fname - w/o extension
  Plot(const std::string& fname_, const std::string& title, const std::string& xlbl, const std::string& ylbl, 
        std::string* legends) : fname(fname_)
  {
    // Plot script
    std::ofstream plt((fname + ".plt").c_str(), std::ofstream::out);
    plt << "#" << std::endl;
    ...
    std::stringstream pltstr;
    pltstr << "plot ";
    // Data file
    of.open((fname + ".dat").c_str());
    of << "#" << std::setw(fw-3) << "X  ";
    for(size_t  lcntr = 0; lcntr < N; lcntr++)
      {
    ...
    plt << pltstr.str();
        ...
        //     << "     '" << fname << ".dat' using 1:4 with lines title label3 lt 3,"
        //     << "     '" << fname << ".dat' using 1:5 with lines title label4 lt 4,"
        //     << "     '" << fname << ".dat' using 1:6 with lines
        //     title label5 lt 5 "<< std::endl;
  ...
  void add(long double x, long double* y)
  {
    ...
    for(size_t ycntr = 0; ycntr < N; ycntr++)
      of << std::setw(fw) << y[ycntr];
