		    ...
		    TSIL_COMPLEX *result)
{
  ...
  if (Y < TSIL_TOL) {
	...
	*result = ( (TSIL_I2(X, Z, U, QQ) - TSIL_I2(Y, Z, U, QQ) + 
		     (Y-X)*TSIL_I2p(Y, Z, U, QQ))/((X-Y)*(X-Y)) );
      else
  ...
  else if (U + TSIL_FABS(Y-Z) < TSIL_TOL) *result = TSIL_Vxy0y(X,Y,S,QQ);
  else if (Z + TSIL_FABS(Y-U) < TSIL_TOL) *result = TSIL_Vxy0y(X,Y,S,QQ);
  else success = 0;
...
TSIL_COMPLEX TSIL_V0x00 (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_REAL lnbarX;  

  if (X < TSIL_TOL) {
    TSIL_Warn ("TSIL_V0x00", "V(0,x,0,0) is undefined for x=0.");
  ...
  if (TSIL_CABS (1.0L - S/X) < 10.0L*TSIL_TOL) {
    TSIL_Warn("TSIL_V0x00", "V(0,x,0,0) is undefined for s=x.");
    return TSIL_Infinity;
  }

  return ((X*TSIL_U0x00(X,S,QQ) -2.0L*TSIL_S000(S,QQ) +TSIL_I200x(X,QQ)
	   + 1.25L*S -X)/(S-X) + TSIL_B0x(X,S,QQ))/X;
}

/* ******************************************************************* */
/* hep-ph/0307101 eq. (6.21)                                           */

TSIL_COMPLEX TSIL_Vxy0y (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S,
			 TSIL_REAL QQ)
{
  TSIL_REAL lnbarY;

  if (Y < TSIL_TOL) {
    TSIL_Warn("TSIL_Vxy0y", "V(x,y,0,y) is undefined for y=0.");
    return TSIL_Infinity;
  }

  if (TSIL_CABS (1.0L - S/(X + Y + 2.0L*TSIL_SQRT(X*Y))) < 10.0L*TSIL_TOL) {
    TSIL_Warn("TSIL_Vxy0y", "V(x,y,0,y) is undefined at s = (sqrt(x)+sqrt(y))^2.");
    return TSIL_Infinity;
  }

  lnbarY = TSIL_LOG(Y/QQ);

  return (TSIL_Tbar0xy(X,Y,S,QQ) - TSIL_Tx0y(Y,X,S,QQ)
	  - lnbarY*TSIL_B(X,Y,S,QQ))/(2.0L*Y) + (lnbarY - 2.0L)*TSIL_Bp(Y,X,S,QQ);
}

/* ******************************************************************* */
/* Special case of hep-ph/0307101 eq. (6.21)                           */

TSIL_COMPLEX TSIL_V0x0x (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_COMPLEX log1mSoX;
  TSIL_REAL    lnbarX;

  if (TSIL_FABS(X) < TSIL_TOL) {
    TSIL_Warn("TSIL_V0x0x", "V(0,x,0,x) is undefined for x=0.");
    return TSIL_Infinity;
  }

  if (TSIL_CABS (1.0L - S/X) < 10.0L*TSIL_TOL) {
    TSIL_Warn("TSIL_V0x0x", "V(0,x,0,x) is undefined for s=x.");
    return TSIL_Infinity;
  }
...
TSIL_COMPLEX TSIL_V0x0y (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_REAL lnbarY;

  if (TSIL_FABS(X) < TSIL_TOL) {
    TSIL_Warn("TSIL_V0x0y", "V(0,x,0,y) is undefined for x=0.");
    return TSIL_Infinity;
  }

  if (TSIL_FABS(Y) < TSIL_TOL)
    return TSIL_V0x00(X, S, QQ);

  if (TSIL_CABS(1-S/X) < 10.0*TSIL_TOL) {
    TSIL_Warn("TSIL_V0x0y", "V(0,x,0,y) is undefined for s=x.");
    return TSIL_Infinity;
  }

  if (TSIL_FABS(X-Y) < TSIL_TOL)
    return TSIL_V0x0x (X, S, QQ);

  lnbarY = TSIL_LOG(Y/QQ);

  return ((S*Y-X*X)*TSIL_U0x0y(X,Y,S,QQ) + Y*(S-Y)*TSIL_Tx00(Y,S,QQ) +
	  X*X -1.25L*S*X + 0.25L*S*Y + 2.L*X*Y - 2.L*Y*Y)/(X*(X - S)*(X - Y))
         + (2.0L*TSIL_S00x(Y,S,QQ) - TSIL_I20xy(X,Y,QQ) - Y*lnbarY)/(X*(X - S))
         + (X - Y*lnbarY + Y)*TSIL_B0x(X,S,QQ)/(X*(X - Y));
}
			 ...
			 TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (TSIL_FABS(X) < TSIL_TOL) {
    TSIL_Warn("TSIL_V0xyz", "V(0,x,y,z) is undefined for x=0.");
    return TSIL_Infinity;
  }
  ...
  if (TSIL_FABS(Z) < TSIL_TOL)
    return TSIL_V0x0y (X, Y, S, QQ);

  if (TSIL_CABS(1.0L - S/X) < TSIL_TOL) {
    TSIL_Warn("TSIL_V0xyz", "V(0,x,y,z) is undefined for s=x.");
    return TSIL_Infinity;
  }
  ...
  if (TSIL_FABS(1.0L - (Y + Z + 2.0L*TSIL_SQRT(Y*Z))/X) < TSIL_TOL)
    return (4.L*TSIL_S0xy(Y,Z,S,QQ) 
          + TSIL_Tx0y(Z,Y,S,QQ)*((4.5L*Z + 1.5L*Y) + ((Z - Y)*(0.5L*S - Z + Y))/X)
          + TSIL_Tx0y(Y,Z,S,QQ)*((4.5L*Y + 1.5L*Z) + ((Y - Z)*(0.5L*S - Y + Z))/X)
          + TSIL_B0x(X,S,QQ)*((X - 2.0L*S) + (X*X + S*(Y-Z))*AY/(2.0L*X*Y) +
  ...
  */
  ...
  return ((-AY*X2 - AZ*X2 - 1.25*S*X2 + X3 + 2.*AY*X*Y + 2.*AZ*X*Y +
	   ...
	   (AY*S*X + AZ*S*X - AY*X2 - AZ*X2 - S*X2 + X3 - AY*S*Y +
	    ...
	    AY*X*Z + AZ*X*Z + S*X*Z - X2*Z)*TSIL_B0x(X,S,QQ) +
	   (-X2 + 2.*X*Y - Y2 + 2.*X*Z + 2.*Y*Z - Z2)*TSIL_I2(X,Y,Z,QQ) +
	   (2.*X2 - 4.*X*Y + 2.*Y2 - 4.*X*Z - 4.*Y*Z + 2.*Z2)*TSIL_S0xy(Y,Z,S,QQ) +
	   (S*X*Y - S*Y2 - X*Y2 + Y3 + S*Y*Z - 3.*X*Y*Z - 2.*Y2*Z + Y*Z2)*
	   TSIL_Tx0y(Y,Z,S,QQ) + (S*X*Z + S*Y*Z - 3.*X*Y*Z + Y2*Z - S*Z2 - X*Z2 -
			     2.*Y*Z2 + Z3)*TSIL_Tx0y(Z,Y,S,QQ) +
	   (-X3 + S*X*Y + X2*Y - S*Y2 + S*X*Z + X2*Z + 2.L*S*Y*Z - S*Z2)*
	   TSIL_U0xyz(X,Y,Z,S,QQ) )/denom);
}
