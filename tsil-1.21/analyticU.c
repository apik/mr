...
int TSIL_Uanalytic (TSIL_REAL X,
		    ...
		    TSIL_COMPLEX *result)
{
  return Uanalytic (X, Y, Z, U, S, QQ, result);
}

/* ******************************************************************* */

int Uanalytic (TSIL_REAL X,
	       TSIL_REAL Y,
	       TSIL_REAL Z,
	       TSIL_REAL U, 
               TSIL_COMPLEX S,
	       ...
	       TSIL_COMPLEX *result)
{
  ...
  if (TSIL_CABS(S) < TSIL_TOL) *result = UAtZero (X, Y, Z, U, QQ);
  else if (X < TSIL_TOL) *result = U0xyz(Y, Z, U, S, QQ);
  else if (U + TSIL_FABS(Y-Z) < TSIL_TOL) *result = Uxy0y (X,Y,S,QQ);
  else if (Z + TSIL_FABS(Y-U) < TSIL_TOL) *result = Uxy0y (X,Y,S,QQ);
  else if (Z + U < TSIL_TOL) *result = Uxy00 (X,Y,S,QQ);
  else if (Y + U < TSIL_TOL) *result = Ux00y (X,Z,S,QQ);
  else if (Y + Z < TSIL_TOL) *result = Ux00y (X,U,S,QQ);
  else if (TSIL_CABS(S-X) + Y + TSIL_FABS(Z-U) < TSIL_TOL) *result = Ux0yyAtx (X,Z,QQ);
  else if (TSIL_CABS(S-U) + Y + TSIL_FABS(X-Z) < TSIL_TOL) *result = Uy0yxAtx (U,X,QQ);
  else if (TSIL_CABS(S-Z) + Y + TSIL_FABS(X-U) < TSIL_TOL) *result = Uy0yxAtx (Z,X,QQ);
  else success = 0;
...
TSIL_COMPLEX U0xyz (TSIL_REAL X, TSIL_REAL Y, TSIL_REAL Z, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (X < TSIL_TOL) return U00xy (Y, Z, S, QQ);
  if (Z < TSIL_TOL) return U0x0y (X, Y, S, QQ);
  if (TSIL_CABS(S) < TSIL_TOL) return ((I20xy(Y,Z,QQ) - I2(X,Y,Z,QQ))/X);
...
TSIL_COMPLEX U00xy (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (X < TSIL_TOL) return U0000(S, QQ);
  if (Y < TSIL_TOL) return U000x(X, S, QQ);
  if (TSIL_FABS(1.0L-Y/X) < TSIL_TOL) return U00xx(X, S, QQ);
  if (TSIL_CABS(S) < 10.0*TSIL_TOL){
    TSIL_Warn("U00xy", "U(0,0,x,y) is undefined for s=0.");
  ...
  return (5.5L - lnbarmS + sqDeltaSXY*(log1mtm - log1mtp)/S 
          ...
          + ((Y-X)/S + (X+Y)/(Y-X))*(Dilog(tm)+ Dilog(tp))   
          + (X-Y)*Dilog(1.0L - Y/X)/S );
}
...
TSIL_COMPLEX U0x0y (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (TSIL_CABS(S) < 10.0*TSIL_TOL)
    return ((I200x(Y,QQ) - I20xy(X,Y,QQ))/X);

  S = AddIeps(S);    
  lnbarY = TSIL_LOG(Y/QQ);  
  result = 5.5L - 2.5L*(X + Y)/S - Zeta2*Y/X - I20xy(X,Y,QQ)/S 
          - Y*Dilog(S/Y)/X + Y*lnbarY*(1.0L/S - 0.5L*lnbarY/X) 
          + (Y/S - 1.0L)*TSIL_CLOG((Y - S)/QQ);

  if (TSIL_CABS(1 - S/X) > 10.0*TSIL_TOL) {
    lnbarXmS = TSIL_CLOG((X-S)/QQ);
    result += (1/X - 1/S)*((X-Y)*Dilog((X-Y)/(X-S))
...
TSIL_COMPLEX U00xx (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (TSIL_CABS(S) < 10.0*TSIL_TOL){
    TSIL_Warn("U00xx", "U(0,0,x,x) is undefined for s=0.");
    return TSIL_Infinity;
  }
...
TSIL_COMPLEX Ux000 (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  return (5.5L + Zeta2 + (1.0L-X/S)*log1mSoX*(log1mSoX - 3.0L + lnbarX)
	  -3.0L*lnbarX + 0.5L*lnbarX*lnbarX + Dilog(S/X));
}

/* ******************************************************************* */
/* hep-ph/0307101 eq. (6.23)                                           */

TSIL_COMPLEX U0x00 (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  return (5.5L + Zeta2 - lnbarmS -2.0L*lnbarX + 0.5L*lnbarX*lnbarX
	  + (1-X/S)*(log1mSoX*(lnbarmS-2.0L)+ Dilog(S/X)));
}
...
TSIL_COMPLEX U000x (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_COMPLEX log1mSoX;
  TSIL_REAL lnbarX;  
  ...
  if (TSIL_CABS(S) < TSIL_TOL){
    TSIL_Warn("U000x", "U(0,0,0,x) is undefined for s=0.");
    return TSIL_Infinity;
  }
  ...
  return (5.5L - Zeta2 + (2.0L*X/S - 2.0L)*log1mSoX +
	  ...
	  -2.0L*lnbarX - 0.5L*lnbarX*lnbarX -(1.0L +X/S)*Dilog(S/X));
...
TSIL_COMPLEX U0000 (TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_COMPLEX lnbarmS;

  if (TSIL_CABS(S) < TSIL_TOL){
    TSIL_Warn("U0000", "U(0,0,0,0) is undefined for s=0.");
    return TSIL_Infinity;
  }
...
TSIL_COMPLEX Uxy0y (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  if (Y < TSIL_TOL) return Ux000 (X, S, QQ);

  return (1.0L - Tx0y(Y,X,S,QQ) + (2.0L - TSIL_LOG(Y/QQ))*B(X,Y,S,QQ));
}

/* ********************************************************************** */
/* R. Scharf and J.B. Tausk, Nucl. Phys. B412, 523 (1994) eqs. (95),(83). */

TSIL_COMPLEX Uxy00 (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (TSIL_FABS(X-Y) < TSIL_TOL) return Uxx00 (X,S,QQ);
  if (X < TSIL_TOL) return U0x00 (Y,S,QQ);  
  if (Y < TSIL_TOL) return Ux000 (X,S,QQ);

  if (TSIL_CABS(S) < TSIL_TOL) return (I2(0,0,Y,QQ) - I2(0,0,X,QQ))/(X-Y);  
  ...
  result = Zeta2 + 5.5L + (X/S -1.0L)*log1msox 
         ...
         + (X - Y - S)*lnbary/S 
         + (Y - X - 2.0L*S)*lnbarx/S 
         + (S + X - Y)/(2.0L*S)*Dilog(sox) 
         + X*((r1 - r2)/(2.0L*S))*
           ((logr1 - logr2)*(2.0L - lnbary) 
           + Dilog(onemr2) - Dilog(onemr1) 
           + Dilog(r1*onem1or2) - Dilog(r2*onem1or1) 
           + EtaBranch(onemsox, 1.0L/r2)*TSIL_CLOG(r1*onem1or2) 
           - EtaBranch(onemsox, 1.0L/r1)*TSIL_CLOG(r2*onem1or1));
...
TSIL_COMPLEX Uxx00 (TSIL_REAL X, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  ...
  if (TSIL_CABS(X/S) < TSIL_TOL) return U0000(S, QQ);
  ...
  return Zeta2 + 5.5L + 0.5L*lnbarx*lnbarx
           ...
           + (1.0L/sox - 1.0L)*TSIL_CLOG(onemsox) + 0.5L*Dilog(sox)
           + 0.5L*sqrtonem4xos*(
             4.0L*TSIL_CLOG(r1) - Dilog(onemr1) + Dilog(onemr2)
             - Dilog(r2*onemr2) + Dilog(r1*onemr1)
             - EtaBranch(r2, onemsox)*TSIL_CLOG(r2*onemr2) +
             + EtaBranch(r1, onemsox)*TSIL_CLOG(r1*onemr1));
}

/* ******************************************************************** */
/* hep-ph/0312092 eq. (A.17)                                            */

TSIL_COMPLEX Ux00y (TSIL_REAL X, TSIL_REAL Y, TSIL_COMPLEX S, TSIL_REAL QQ)
{
  TSIL_COMPLEX AX, AY, B0X, I200Y, S0XY, TX0Y, TY0X;

  if (Y < TSIL_TOL) return Ux000(X, S, QQ);
  if (X < TSIL_TOL) return U000x(Y, S, QQ);

  I200Y = I2(0,0,Y,QQ);

  if (TSIL_CABS(S/X) < 10.0L*TSIL_TOL) return (I200Y - I2(0,X,Y,QQ))/X; 
...
TSIL_COMPLEX Ux0yyAtx (TSIL_REAL X, TSIL_REAL Y, TSIL_REAL QQ)
{
  TSIL_REAL lnbarX, lnbarY;
  
  if (X < 10.0*TSIL_TOL){
    TSIL_Warn("U0x0yyAtx", "U(x,0,y,y) is undefined for s=x=0.");
    return TSIL_Infinity;
  }

  if (Y < TSIL_TOL) return Ux000(X, X, QQ);

  lnbarX = TSIL_LOG(X/QQ);
  lnbarY = TSIL_LOG(Y/QQ);

  return (5.5L + (1.0L + Y/X)*Zeta2 - 3.0L*lnbarX + lnbarX*lnbarY
          - 0.5L*lnbarY*lnbarY - (1.0L +Y/X)*Dilog(1.0L- X/Y)
          - 4.0L*fPT(X,Y) );
}
...
TSIL_COMPLEX fPT(TSIL_REAL x, TSIL_REAL y)
{
  ...
  return ((sqrty/sqrtx)*(1.5L*Zeta2 + Dilog(argdilog) - Dilog(-argdilog)));
}
...
TSIL_COMPLEX Uy0yxAtx (TSIL_REAL X, TSIL_REAL Y, TSIL_REAL QQ)
{
  TSIL_REAL lnbarX, lnbarY;
  TSIL_COMPLEX  log1mXoY;

  if (X < TSIL_TOL) return Ux00y(Y, Y, 0, QQ);
  if (Y < TSIL_TOL) return U000x(X, X, QQ);

  lnbarX = TSIL_LOG(X/QQ);
  lnbarY = TSIL_LOG(Y/QQ);
  log1mXoY = TSIL_CLOG(1.0L - AddIeps(X/Y));

  return 5.5L - 2.0L*lnbarX - lnbarY + lnbarY*lnbarY/2.0L
          + Dilog(1-X/Y)*(1.0L + 2.0L*Y/X) -2.0L*(1 + Y/X)*Zeta2
	...
	if (TSIL_FABS(y - z) > 0.0L) { /* Use A17 */
	  foo->U[i].value = 
	    ...
	    (2.0L * x * *(foo->U[i].tval[0]) + 2.0L * *(foo->U[i].sval) - 
	     I2(0, y, z, qq) - A(x, qq) - A(y, qq) - A(z, qq) 
             + x + y + z - s/4.0L)/(s - x) +
	     B0x (x, s, qq)*(A(y, qq) - A(z, qq))/(z - y);
	}
	else { /* Use A18 */
	  foo->U[i].value =
	    ((s - x - 4.0L*y) * *(foo->U[i].tval[2]) -
	     ...
	     4.0L * *(foo->U[i].sval) +
	     2.0L*I2(0, y, y, qq) + 2.0L*A(x, qq) + 4.0L*A(y, qq) 
             - 3.0L*x - 4.0L*y + 1.5L*s)/(x - s) 
	     - (1.0L + A(y, qq)/y)*B0x (x, s, qq);
	}
      }
      else {
      /* The s = x case */
        if (TSIL_FABS(y - z) > 0.0L)
  	  foo->U[i].value = Ux0yzAtx (x, y, z, foo->U[i].tval, qq);
        else
	  foo->U[i].value = Ux0yyAtx (x, y, qq );    
      }
    }
  }
...
TSIL_COMPLEX Ux0yzAtx (TSIL_REAL x,
		       TSIL_REAL y,
		       TSIL_REAL z,
		       TSIL_COMPLEX **tptr,
		       TSIL_REAL qq)
{
  return (-2.0L*A(y, qq) + 2.0L*A(x, qq)*A(y, qq)/x + 
	  2.0L*A(z, qq) - 2.0L*A(x, qq)*A(z, qq)/x +
...
TSIL_COMPLEX UAtZero (TSIL_REAL x,   
		      TSIL_REAL y,
		      TSIL_REAL z,
		      TSIL_REAL u,
		      TSIL_REAL qq)
{
  if (TSIL_FABS(x - y) > TSIL_TOL)
    return (I2(x, z, u, qq) - I2(y, z, u, qq))/(y - x);
  else
    return -I2p(x, z, u, qq);
}
...
TSIL_COMPLEX UprimeAtZero (TSIL_REAL x,
			   TSIL_REAL y,
			   TSIL_REAL z,
			   TSIL_REAL u,
			   TSIL_REAL qq)
{
  ...
  if (x < TSIL_TOL) {
    ...
    return z*I2p2(z,0,u,qq);
  }  
    
  if (TSIL_FABS(x - y) > TSIL_TOL)
    return x*(I2(x, z, u, qq) - I2(y, z, u, qq))/TSIL_POW(y - x, 3)
      + x*I2p(x, z, u, qq)/TSIL_POW(y - x, 2)
      + x*I2p2(x, z, u, qq)/(2.0L*(y - x));
  else
    return -x*I2p3(x, z, u, qq)/6.0L;
}
