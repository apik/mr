...
namespace mr
{
  ...
  const double Zeta3 = 1.20205690315959428539973816151144999076498629234049888179227155534L;
  const double Zeta4 = (Pi*Pi*Pi*Pi)/90.;
  const double Zeta5 = 1.03692775514336992633136548645703416805708091950191281197419267790L;
  const double Zeta6 = (Pi*Pi*Pi*Pi*Pi*Pi)/945.;
  const double Zeta7 = 1.00834927738192282683979754984979675959986356056523870641728313657L;

  // a4 = Li[4,1/2]
  const double a4    = 0.5174790616738993863307581618988629456223774751413792582443193479770L;
  // a5 = Li[5,1/2]
  const double a5    = 0.5084005792422687074591088492585899413195411256648216487244977963526L;
  
  const double S1    = Pi/sqrt(3.);
  const double S2    = 0.26043413763216209895572914320803078545504477884842847340736668765L;
  
  // g5 = -i/24*EPAIR*eps(mu1,mu2,mu3,mu4)*g(mu1)*g(mu2)*g(mu3)*g(mu4)
  // Sp[g5*g5] = 4
  const double EPAIR2 = -1.;
