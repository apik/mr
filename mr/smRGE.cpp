...
namespace mr
{
  ...
  BetaSMFull::BetaSMFull(int pocoa1_, int pocoa2_, int pocoas_, int pocoat_, int pocoab_, int pocoatau_, int pocolam_,int NG_, bool MultiplyByMinus1_) : NG(NG_), pocoa1(pocoa1_), pocoa2(pocoa2_), pocoa3(pocoas_), pocoa4(pocoat_), pocoa5(pocoab_), pocoa6(pocoatau_), pocoa7(pocolam_), MultiplyByMinus1(MultiplyByMinus1_)
  {
    ...
    // 5-loop QCD correction
    add(be4, pocoa4, 0, 0, 5, 1, 0, 0, 0,      
        -99512327/81. - (92804932*Zeta3)/243. - 193600*pow(Zeta3,2) + 
        (1396252*Zeta4)/9. + (463514320*Zeta5)/243. + 
        pow(NG,4)*(34.23868312757202 + (10240*Zeta3)/243. - (2048*Zeta4)/27.) + 
        pow(NG,3)*(-1008.1207133058984 - (205568*Zeta3)/81. - (7168*Zeta4)/9. +  (81920*Zeta5)/27.) + 
        pow(NG,2)*(-14493.739368998627 - (16086592*Zeta3)/243. - 
                   (371200*pow(Zeta3,2))/27. + (1330400*Zeta4)/27. + (2112320*Zeta5)/81. - 
                   (736000*Zeta6)/27.) - 484000*Zeta6 - 825440*Zeta7 + 
        NG*(413542.6145404664 + (50152064*Zeta3)/81. + (302720*pow(Zeta3,2))/9. - 
            (8154968*Zeta4)/27. - (199504720*Zeta5)/243. + (2552000*Zeta6)/9. + 
            (7280000*Zeta7)/27.));
    ...
    // 4-loop QCD correction
    add(be5, pocoa5, 0, 0, 4, 0, 1, 0, 0,      
        -4603055./81. + (366892.*NG)/27. - (41936.*pow(NG,2))/243. + (5312.*pow(NG,3))/243. -
        3520.*NG*Zeta4 + (1280.*pow(NG,2)*Zeta4)/3. - (271360.*Zeta3)/27. +
        (136768.*NG*Zeta3)/9. - (6400.*pow(NG,2)*Zeta3)/9. - (1024.*pow(NG,3)*Zeta3)/27. +
        17600.*Zeta5 - (73600.*NG*Zeta5)/9.);
    // 5-loop QCD correction
    add(be4, pocoa4, 0, 0, 5, 0, 1, 0, 0,      
        -99512327/81. - (92804932*Zeta3)/243. - 193600*pow(Zeta3,2) + 
        (1396252*Zeta4)/9. + (463514320*Zeta5)/243. + 
        pow(NG,4)*(34.23868312757202 + (10240*Zeta3)/243. - (2048*Zeta4)/27.) + 
        pow(NG,3)*(-1008.1207133058984 - (205568*Zeta3)/81. - (7168*Zeta4)/9. +  (81920*Zeta5)/27.) + 
  ...
  double BetaSM::gamv(const SMCouplings &a, size_t NG, int poco)
  {
    ...
    if(poco >= 1)
      {
        // 1-loop
        gamV += pow(a[6],-1) * ( 1./2.*pow(a[5],2) + 3./2.*pow(a[4],2) + 3./2.*
